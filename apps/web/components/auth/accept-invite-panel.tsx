...
export function AcceptInvitePanel({
  ...
  const isWrongAccount =
    ...
    viewer!.user.email.trim().toLowerCase() !==
      invitation!.email.trim().toLowerCase();

  if (!invitationId) {
    return (
      <p className="text-sm text-red-600 dark:text-red-400">
        Invitation id missing from the URL.
      </p>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
          Sign in or create an account with the invited email address before
          accepting this invite.
        </p>
        <div className="flex flex-wrap gap-3">
          <Link
            href={
              `/sign-in?redirectTo=${encodeURIComponent(redirectTo)}` as never
            }
          >
  ...
  if (isWrongAccount) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-red-600 dark:text-red-400">
          This invitation is for {invitation.email}. Switch accounts to
          continue.
        </p>
