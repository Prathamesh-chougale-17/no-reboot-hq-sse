  ...
  router.post(
    ...
    zValidator("json", CreateWebhookEndpointInputSchema, (result) => {
      if (!result.success) {
        throw new AppError(
          400,
          "VALIDATION_ERROR",
          "Webhook endpoint payload is invalid",
          {
    ...
    async (c) => {
      ...
      return jsonSuccess(
        c,
        201,
        CreateWebhookEndpointResultDtoSchema.parse(endpoint),
      );
    },
  );
  ...
  router.delete(
    ...
    zValidator("param", WebhookEndpointParamsSchema, (result) => {
      if (!result.success) {
        throw new AppError(
          400,
          "VALIDATION_ERROR",
          "Webhook endpoint id is invalid",
          {
            issues: result.error.issues,
          },
        );
    ...
    async (c) => {
      ...
      return jsonSuccess(
        c,
        200,
        DeleteWebhookEndpointResultDtoSchema.parse(deleted),
      );
    },
  );
