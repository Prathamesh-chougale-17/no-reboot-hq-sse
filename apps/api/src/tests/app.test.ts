...
vi.mock("@acme/auth", () => ({
  ...
  resolveAuthContext: vi.fn(async (headers: Headers) =>
    headers.get("cookie")?.includes("session=valid")
      ? currentAuthContext
      : null,
  ),
  ...
  requireRole: vi.fn(async (headers: Headers, roles?: readonly string[]) => {
    ...
    if (
      roles &&
      (!currentAuthContext.role || !roles.includes(currentAuthContext.role))
    ) {
  ...
  return {
    usersRepository: {
      hasAnyMembership: vi.fn(
        async () =>
          Boolean(currentAuthContext.organizationId) ||
          currentAuthContext.organizations.length > 0,
      ),
...
describe("api routes", () => {
  ...
  it("returns health metadata", async () => {
    ...
    expect(body.data.service).toBe("no-reboot-hq-api");
  });
  ...
  it("serves an OpenAPI document for the public API surface", async () => {
    ...
    expect(body.info.title).toBe("No Reboot HQ API");
  ...
  it("serves Scalar docs for the public API surface", async () => {
    ...
    expect(html).toContain("No Reboot HQ API");
  ...
  it("creates invitations with validated payloads and appends an audit log", async () => {
    ...
    const body =
      (await response.json()) as ApiResponse<CreateInvitationResultDto>;
    ...
    expect(repositories.auditRepository.appendAuditLog).toHaveBeenCalledTimes(
      1,
    );
  ...
  it("creates workspaces through the server-owned endpoint and appends an audit log", async () => {
    ...
    const body =
      (await response.json()) as ApiResponse<CreateWorkspaceResultDto>;
  ...
  it("accepts invitations through the server-owned endpoint and appends an audit log", async () => {
    ...
    const body =
      (await response.json()) as ApiResponse<AcceptInvitationResultDto>;
  ...
  it("returns newest-first audit logs for owners and admins", async () => {
    ...
    expect(
      repositories.auditRepository.listOrganizationAuditLogs,
    ).toHaveBeenCalledWith(baseOrganization.id, 25);
  });
  ...
  it("lists webhook endpoints for owners and admins", async () => {
    ...
    expect(body.data.items).toHaveLength(1);
    expect(
      repositories.webhookRepository.listOrganizationWebhookEndpoints,
    ).toHaveBeenCalledWith(baseOrganization.id);
  });
  ...
  it("creates webhook endpoints and returns the signing secret once", async () => {
    ...
    const body =
      (await response.json()) as ApiResponse<CreateWebhookEndpointResultDto>;
    ...
    expect(
      repositories.webhookRepository.createWebhookEndpoint,
  ...
  it("deletes webhook endpoints for owners and admins", async () => {
    ...
    const response = await app.request(
      "/api/v1/webhooks/2d2dbf71-83df-4a9c-9f4e-b525857a7e60",
      {
        method: "DELETE",
        headers: {
          cookie: "session=valid",
    ...
    const body =
      (await response.json()) as ApiResponse<DeleteWebhookEndpointResultDto>;
    ...
    expect(
      repositories.webhookRepository.deleteWebhookEndpoint,
