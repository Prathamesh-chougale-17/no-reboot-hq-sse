...
describe("config", () => {
  it("parses api env with defaults", () => {
    ...
    expect(env.API_SERVICE_NAME).toBe("no-reboot-hq-api");
    expect(env.REDIS_PREFIX).toBe("no-reboot-hq");
  ...
  it("parses better-auth env with defaults", () => {
    ...
    expect(env.AUTH_FROM_EMAIL).toContain("auth@no-reboot-hq.local");
    expect(env.REDIS_PREFIX).toBe("no-reboot-hq");
  });
  ...
  it("parses async platform env values with defaults", () => {
    ...
    expect(env.REDIS_PREFIX).toBe("no-reboot-hq");
    expect(env.CONFIG_EVENTS_TOPIC).toBe("config.events");
  });
