...
export type WebhookDeliveryJobPayload = z.infer<
  typeof WebhookDeliveryJobPayloadSchema
>;
...
const getQueue = (name: QueueName): Queue => {
  ...
  if (!env.REDIS_URL) {
    throw new Error(
      `Cannot create queue "${name}" because REDIS_URL is not configured.`,
    );
  }
...
export const enqueueInviteEmailJob = async (
  payload: InviteEmailJobPayload,
): Promise<void> => {
  const parsed = InviteEmailJobPayloadSchema.parse(payload);

  await getQueue(JOB_QUEUE_NAMES.inviteEmail).add(
...
export const enqueueWebhookDeliveryJob = async (
  ...
  await getQueue(JOB_QUEUE_NAMES.webhookDelivery).add(
...
export const closeRedisConnections = async (): Promise<void> => {
  await Promise.all(
    [...producerConnectionCache.values()].map((connection) =>
      connection.quit(),
    ),
  );
